...
export const DiscoverProjectsPage = () => {
  ...
  const { items: allProjects = [], status, error } =
    ...
    return [...allProjects]
      ...
      .filter((project) => {
        ...
        const matchesStage =
          ...
          project.stage?.toLowerCase() === activeStage.toLowerCase()

        return matchesSearch && matchesLocation && matchesStage
      })
  }, [allProjects, searchQuery, locationQuery, activeStage])

  const now = new Date()

  const featuredProjects = sortedAndFilteredProjects.filter(
    (p) => p.isBoosted && new Date(p.boostExpiresAt) > now
  )

  const latestProjects = sortedAndFilteredProjects.filter(
    (p) => !p.isBoosted || new Date(p.boostExpiresAt) <= now
  )
    ...
    <div className={styles.pageContainer}>
      <header className={styles.header}>
        ...
        <p className={styles.subtitle}>
          Join high impact projects, connect with visionary founders, and build
          the future together.
        </p>
          ...
          {stages.map((stage) => (
            <button
              ...
              className={`${styles.filterChip} ${
                activeStage === stage ? styles.activeChip : ''
              }`}
